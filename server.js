...
var server = http.createServer(function (request, response) {
    ...
    if (path === '/sign_in' && method === 'POST') {
        ...
        // get请求通过query拿到
        // post请求获取数据
        const array = []//新建一个空数据，不知道数据是不是一段一段的
        request.on('data', (chunk) => {
            array.push(chunk)//chunk片段；获取数据，一点一点读取到数组
        })
        request.on('end', () => {
            // console.log(array)
            const string = Buffer.concat(array).toString()//转换成字符串，utf-8编码
            const obj = JSON.parse(string)
            // 存数据库
            const lastUser = userArray[userArray.length - 1]//最后一个用户
            // id为最后一个用户的id加一；判断是不是没有用户，空，则id=1
            const newUser = {
